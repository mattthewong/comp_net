...
class DataCenter(Topo):
    ...
    def __init__(self, n=1, delay='0ms', fi=1,  cpu=.01, max_queue_size=None, **params):
        ...
        midLevelSwitchSet = []
        for i in range(fi):
            mls = self.addSwitch('mls'+str(i+1))
            # Create link between tls and mls
            self.addLink(tls, mls, **swlinkConfig)
            for j in range(fi):
                lls = self.addSwitch('s'+str(i+1)+'x'+str(j+1))
                # Create link between each mls and lls
                self.addLink(mls, lls,**swlinkConfig)
                for k in range(n):
                    host = self.addHost('h'+str(i+1)+'x'+str(j+1)+'x'+str(k+1))
                    # Create link between each mls and lls
                    self.addLink(lls, host,**hostlinkConfig)
